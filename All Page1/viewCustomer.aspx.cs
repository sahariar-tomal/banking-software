...
public partial class viewCustomer : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (type.Equals("Accountant") || type.Equals("Cash Officer"))
        {
        ...
        if (Page.IsPostBack == false)
        {
            try
            {
                ...
                Image1.ImageUrl = "image/viewcustpp.ashx?sid=" + Convert.ToInt16(Session["sid"]);
                //introaccount = Session["introid"].ToString();
            }
            catch (Exception ie)
            {
                Response.Redirect("~/login.aspx");
                //Label2.Text = "Cant access the intro session";
            }
            ...
            query1 = "select * from Customer where cid='" + Session["sid"].ToString() + "'";
            query2 = "select * from presentaddress where cid='" + Session["sid"].ToString() + "'";
            query3 = "select * from permanentaddress where cid='" + Session["sid"].ToString() + "'";
            query5 = "select * from introducer where introduced_account=" + Session["sid"].ToString();
            //query4 = "select * from introducer where intro_account='" + introaccount + "'";
            ...
            TextBox41.Text = dman.ReadRow(query5, connection)["intro_name"].ToString();
            TextBox42.Text = dman.ReadRow(query5, connection)["intro_account"].ToString();
    ...
    {
        string upquery1 = "update Customer set mobile1='" + TextBox17.Text + "'," + "mobile2='" + TextBox18.Text + "'," + "tele='" + TextBox19.Text + "'," + "email='" + TextBox20.Text + "' where cid ="+Session["sid"].ToString();
        string upquery2 = "update presentaddress set paddress_hno='" + TextBox29.Text + "'," + "paddress_rno='" + TextBox30.Text + "'," + "village='" + TextBox31.Text + "'," + "thana='" + TextBox32.Text + "'," + "district='" + TextBox33.Text + "'," + "pcode='" + TextBox34.Text + "' where cid="+Session["sid"].ToString();
        string upquery3 = "update permanentaddress set per_hno='" + TextBox35.Text + "'," + "per_rno='" + TextBox36.Text + "'," + "per_vil='" + TextBox37.Text + "'," + "per_thana='" + TextBox38.Text + "'," + "per_dis='" + TextBox39.Text + "'," + "per_pcode='" + TextBox40.Text + "' where cid ="+Session["sid"].ToString();
        ...
        OracleCommand cmd1 = new OracleCommand(upquery1, connection);
        cmd1.ExecuteNonQuery();

        OracleCommand cmd2 = new OracleCommand(upquery2, connection);
        cmd2.ExecuteNonQuery();

        OracleCommand cmd3 = new OracleCommand(upquery3, connection);
        cmd3.ExecuteNonQuery();

        dman.connClose(connection);

       // Label2.Text = upquery1;//+ upquery2 + upquery3;
    }
}